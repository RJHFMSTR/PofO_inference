...
void Engine::IBDSegInSubset64Bit(IntArray & pairList, Vector & ibdprops, Vector & maxLengths, Vector & ibd2props, Vector & maxLengths2, unsigned long long int **localLG[2], int localmarkerCount, IntArray & localchrSeg, double localtotalLength, IntArray &localbp, IntArray *ibd1segs, IntArray *ibd2segs)
{
      ...
      skipFlag, localMin, localMax, minExtraBit, maxExtraBit, cCount, icCount)
{
   ...
   for(int p = 0; p < pairCount; p++){
      ...
      for(int seg = 0; seg < segCount; seg++){
         int chrsegMin = localMin = localchrSeg[seg<<2];
         int chrsegMax = localMax = localchrSeg[(seg<<2)|1];
         ...
         if(localMin >= chrsegMax || localbp[(chrsegMax<<6)|0x3F] - localbp[localMin<<6] < 2500000)
      ...
      for(int k = 0; k < 2; k++){
         ...
         if(newsegCount){
            ...
            for(int seg = 0; seg < newsegCount; seg++){
               ...
               if(stopPos[k][seg] == locallongCount-1)
                  localMax = localmarkerCount - 1;
               else
                  localMax = ((stopPos[k][seg]<<6)|0x3F)+stopExtraBit[k][seg];
               length = localbp[localMax] - localbp[localMin];
