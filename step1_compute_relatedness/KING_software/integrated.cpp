...
void Engine::IntegratedRelationshipInference()
{
   ...
   if(totalLengthX>0)
      for(int f = 0; f < ped.familyCount; f++){
         ...
         for(int i = 0; i < id[f].Length(); i++)
            for(int j = i+1; j < id[f].Length(); j++)
               phiXs.Push(kinX(ped[id[f][i]], ped[id[f][j]]));
      }
   ...
   if(pairCount){ // within-family inference
         ...
         if(IBDvalidFlag){ // IBD seg
            ...
            if(type=="Dup/MZ"){
               ...
               if(inflation > 2 || inflation < 0.5)
                  ...
                  if(phis[p]==0.25 && pi0s[p]==0)  // PO
                     errorFlag = 1;
                  else if(phis[p]==0.25 && pi0s[p]==0.25 && ibd2prop < 0.05)// FS
                     errorFlag = 1;
                  else
                     errorFlag = 0.5;
               }else
      ...
      fclose(fp);                                                                            
      if(IBDvalidFlag && Bit64 == 64 && totalLengthX > 0){
         IBDSegInSubset64Bit(pairs, ibdprops, maxLengths, ibd2props, maxLengths2, XLG, xmarkerCount, chrSegX, totalLengthX, xbp);
         outfile=prefix;  outfile.Add("X.kin");
         fp = fopen(outfile, "wt");
         fprintf(fp, "FID\tID1\tID2\tSex1\tSex2\tPhiX\tIBD1Seg\tIBD2Seg\tPropIBD\n");
         for(int p = 0; p < pairCount; p++){
            if(!het1Counts[p] && !het2Counts[p] && !HetHetCounts[p]) continue;
            id1 = pairs[p*2]; id2 = pairs[p*2+1];
            fprintf(fp, "%s\t%s\t%s\t%d\t%d\t%.4lf\t%.4lf\t%.4lf\t%.4lf\n",
               (const char*)ped[phenoid[id1]].famid,
               (const char*)ped[phenoid[id1]].pid,
               (const char*)ped[phenoid[id2]].pid,
               ped[phenoid[id1]].sex, ped[phenoid[id2]].sex, phiXs[p],
               ibdprops[p], ibd2props[p], ibd2props[p] + ibdprops[p]*0.5);
         }  // end of pairs
         fclose(fp);
         printf("Within-family X-chr IBD-sharing inference saved in file %s\n", (const char*)outfile);
      }
   }else
   ...
   if(Bit64==64){
      if (relativedegree < 3) {
         if (longCount < 65) midrelativeCount = -1;
         else ScreenCloseRelativesInSubset64Bit(allpairs0);
      }else // more distant relatives
         ComputeLongRobustKinship64BitWithFilter(allpairs0, false);
   }
   else {
      if (shortCount < 257) midrelativeCount = -1;
      else ScreenCloseRelativesInSubset(allpairs0);
   }
   //****************SCREENING RELATIVES end************************

   if (midrelativeCount != -1) {
   ...
   }else { // midrelative==-1, not screened
      for(int t = 0; t < defaultMaxCoreCount; t++)
         allpairs[t].Dimension(0);
   ...
   for(int j = 0; j < 6; j++)
      afterCount[j] = 0;
   int fpCount = 0;
   String outfile2(prefix); outfile2.Add("X.kin0");
   FILE *fp2=NULL;
   if(IBDvalidFlag && Bit64 == 64 && totalLengthX > 0){
      fp2 = fopen(outfile2, "wt");
      fprintf(fp2, "FID1\tID1\tFID2\tID2\tSex1\tSex2\tIBD1Seg\tIBD2Seg\tPropIBD\n");
   }
   for(int t = 0; t < defaultMaxCoreCount; t++){
      ...
      if(IBDvalidFlag && Bit64 == 64 && totalLengthX > 0){
         IBDSegInSubset64Bit(allpairs[t], ibdprops, maxLengths, ibd2props, maxLengths2, XLG, xmarkerCount, chrSegX, totalLengthX, xbp);
         for(int p = 0; p < pairCount; p++){
            if(!het1Counts[p] && !het2Counts[p] && !HetHetCounts[p]) continue;
            id1 = allpairs[t][p*2]; id2 = allpairs[t][p*2+1];
            if(ped[phenoid[id1]].famid==ped[phenoid[id2]].famid) continue;
            fprintf(fp2, "%s\t%s\t%s\t%s\t%d\t%d\t%.4lf\t%.4lf\t%.4lf\n",
               (const char*)ped[phenoid[id1]].famid, (const char*)ped[phenoid[id1]].pid,
               (const char*)ped[phenoid[id2]].famid, (const char*)ped[phenoid[id2]].pid,
   ...
   if (IBDvalidFlag && Bit64 == 64 && totalLengthX > 0)
       printf("  X-Chr IBD-sharing inference saved in file %s\n", (const char*)outfile2);
   if(relativedegree == 1){
