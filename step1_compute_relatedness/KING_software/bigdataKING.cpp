...
void Engine::ComputeInnerProduct64Bit(IntArray & subset, IntArray * counts)
{
   ...
   int *missingInOnePersonCount = new int[subsetCount];
#ifdef _OPENMP
   ...
   for(int i = 0; i < subsetCount; i++){
      ...
      for(int m = 0; m < longCount; m++){
         ...
         for (word = ~(LG[0][id][m] | LG[1][id][m]); word; word &= (word - 1), sum++);
      }
   ...
   int localMiss[BLOCKSIZE][BLOCKSIZE];
   ...
   #pragma omp parallel for num_threads(defaultMaxCoreCount) \
      private(word0, word1, word2, word, localD, localMiss)
#endif
   for(int k = 0; k < loopIndexLength; k++){
      ...
      for(int ii = 0; ii < BLOCKSIZE; ii++)
         for(int jj = 0; jj < BLOCKSIZE; jj++)
            localD[ii][jj] = localMiss[ii][jj] = 0;
      for(int mm = 0; mm < longCount; mm += CACHESIZE){
         ...
         for(int i1 = i; i1 < iMax; i1++){
            ...
            for(int i2 = j; i2 < jMax; i2++){
               ...
               localMiss[ii][jj] += missmissCount;  // MissMiss
            }  // end of id2
         }  // end of id1
      }// end of mm
      for(int id1 = i; id1 < iMax; id1++){
         ...
         for(int id2 = jMin; id2 < jMax; id2++){
            char jj = id2 - j;
            counts[id1][id2] = localD[ii][jj];
            counts[id2][id1] = markerCount - missingInOnePersonCount[id1] - missingInOnePersonCount[id2] + localMiss[ii][jj];
         }  // end of id2
      }  // end of id1
   }  // end of pairs
...
void Engine::ComputeDistanceMatrix64Bit(IntArray & subset, Matrix & Dist)
{
   int subsetCount = subset.Length();
   if (subsetCount == 0) {
       for (int i = 0; i < idCount; i++) subset.Push(i);
       subsetCount = idCount;
   }
   int notMissingCount;
   ...
   for(int i = 0; i < subsetCount; i++){
       ...
       for(int m = 0; m < longCount; m++)   // not all non-missing
           for(unsigned long long int word = ~(LG[0][id][m] | LG[1][id][m]); word; word &= (word-1), sum++);
       missingInOnePersonCount[i] = sum;
   }  // parallel among individuals ends
   Dist.Dimension(subsetCount, subsetCount);
   Dist.Zero();
   const int BLOCKSIZE=8;
   const int CACHESIZE=512;   // 2^(9+4+4)=128K total cache
...
#ifdef _OPENMP
   #pragma omp parallel for num_threads(defaultMaxCoreCount) \
      private(notMissingCount, word1, word2, word, localD)
#endif
   for(int k = 0; k < loopIndexLength; k++){
      int i = loopIndex[0][k];
      int iMax = i<subsetCount-BLOCKSIZE? i+BLOCKSIZE: subsetCount;
      ...
      for(int i1 = i; i1 < iMax; i1++){
         //if(missingInOnePersonCount[i1]>=cutoffMissingCount) continue;
         char ii = i1 - i;
         if(i == j) jMin = i1 + 1;
         for(int i2 = jMin; i2 < jMax; i2++){
            char jj = i2 - j;
            //if(missingInOnePersonCount[i2]>=cutoffMissingCount) continue;
            localD[1][ii][jj] += markerCount - missingInOnePersonCount[i1] - missingInOnePersonCount[i2];
            Dist[i2][i1] = Dist[i1][i2] = (localD[0][ii][jj] * 1.0 / localD[1][ii][jj]);
         }  // end of id2
      }  // end of id1
   }  // end of pairs
   delete []missingInOnePersonCount;
...
for (int i = 0; i < subsetCount; i++)
    if (missingInOnePersonCount[i] < cutoffMissingCount)
        subset1.Push(subset[i]);
if (subset1.Length() < subsetCount) {
    subset = subset1;
