...
{
#ifndef VERSION
   printf("KING - (c) 10/11/2019 Wei-Min Chen");
#else
...
int main(int argc, char **argv)
{
   ...
   BEGIN_LONG_PARAMETERS(longParameters)
      ...
      LONG_PARAMETER_GROUP("Structure Parameter")
         LONG_INTPARAMETER("projection", &projectionFlag)
         LONG_INTPARAMETER("pcs", &pcs)
#ifndef VERSION
         LONG_STRINGPARAMETER("popref", &popreffile)
#endif
...
//         LONG_STRINGPARAMETER("window", &windowPar)
      ...
      LONG_PARAMETER_GROUP("Output")
         ...
         LONG_STRINGPARAMETER("rpath", &rpath)
   ...
   if (pcs)
       engine.nPC = pcs;
   if(projectionFlag) {
      engine.projectFlag = true;
      if(projectionFlag>1) engine.projectStart = projectionFlag;
   }
   ...
   if(flags[KinshipFLAG] || flags[IBSFLAG] || flags[HomoFLAG] || flags[RiskFLAG] ||
      flags[BysnpFLAG] || flags[BysampleFLAG] || flags[AutoQCFLAG] || IBDSegFLAG || 
   ...
   if (rplotFlag) {
      engine.rplotFlag = true;
      if (rpath.IsEmpty()) rpath = "R";
   }
   if(!popreffile.IsEmpty()) engine.popreffile = popreffile;
#ifndef VERSION
...
//   if(runlengthFlag) engine.runlengthFlag = true;
   ...
   || flags[ClusterFLAG] || flags[BuildFLAG] || flags[UnrelatedFLAG] || IBDAnalysis || bpekingFlag || flags[AutoQCFLAG] || individual) {
   ...
   if(Bit64 == 64){  // --sysbit 64
      if(Bit64Flag && (allflags & (base[AutoQCFLAG] | base[HomoFLAG]))){
...
//      engine.Bit64 = 32;
   ...
   if(!allflags && rplotFlag){
      if(engine.SplitPedigree()) plotSplitped(prefix, rpath);
      exit(0);
   }
#endif
   for(int flag = 0; flag < TOTALFLAGCOUNT; flag++)
      if(flags[flag])
          ...
          case ClusterFLAG:
              ...
              if (flags[BysampleFLAG] || flags[BysnpFLAG] || flags[TDTFLAG]) {
                  ...
                  if (rplotFlag && Bit64 == 64 && engine.totalLength > 10000000) plotCluster(prefix, rpath);
              }
              break;
          case BysampleFLAG:
          ...
          case MDSFLAG:
              if (Bit64 == 64) {
                  if (projectionFlag) engine.mds_projection(true);
                  else if (familyData) {
                      printf("Families are detected. Reported pedigrees will be incorporated in MDS.\n");
                      engine.mds_family64Bit(true);
                  }
                  else engine.mds();
              }
              else {   // 32-bit
                  if (projectionFlag)
                      printf("Projection flag ignored for 32-bit version.\n");
                  engine.mds();
              }
              if (rplotFlag) {
                  if (projectionFlag) {
                      if (plotAncestry(prefix, rpath))   // if failed
                          plotPopStructure(prefix, projectionFlag, rpath);
                  }
                  else plotPopStructure(prefix, projectionFlag, rpath);
              }
              break;
          case RelatedFLAG:
              if (Bit64 == 32 && (allflags & (base[AutoQCFLAG] | base[HomoFLAG]))) {
                  ...
                  if (engine.idCount < 10) {
                      ...
                      if (rplotFlag) {
                          plotRelationship(prefix, rpath);
                          if (engine.SplitPedigree()) plotMIerror(prefix, rpath);
                          plotUniqueFamily(prefix, degree == 0 ? 1 : degree, "related", rpath);
                      }
                  }
              }
              break;
          case DuplicateFLAG:
              ...
              if (rplotFlag) plotDuplicate(prefix, rpath);
              break;
          case UnrelatedFLAG:
          {
              ...
              engine.ExtractUnrelated(unrelatedList, true);
          ...
          case BuildFLAG:
              ...
              if (rplotFlag && built) plotBuild(prefix, rpath);
              break;
          case PCAFLAG:
              if (Bit64 == 64) {
                  if (projectionFlag) engine.mds_projection(false);
                  else if (familyData) {
                      printf("Families are detected. Reported pedigrees will be incorporated in PCA.\n");
                      engine.mds_family64Bit(false);
                  }
                  else engine.pca64Bit();
              }
              else {   // 32-bit
          ...
          case IBDSegFLAG:
               ...
               if(rplotFlag) {
                  plotIBDSeg(prefix, rpath);
                  if(degree) plotUniqueFamily(prefix, degree, "ibdseg", rpath);
                  else
          ...
          case ROHFLAG:
               if(Bit64==64){
                  engine.ROH();
                  if(rplotFlag) plotROH(prefix, rpath);
               }else{
   ...
   if(IBDAnalysis){
      if(LinkageNPL){
         engine.NPL();
         if(rplotFlag) plotNPL(prefix, sexchr, rpath);
      }
      if(LinkageHE){
         engine.HEreg();
         if(rplotFlag) plotHEreg(prefix, sexchr, rpath);
      }
      if(LinkageIBD){
         engine.IBDmapping(permCount);
         if(rplotFlag) plotIBDmapping(prefix, sexchr, rpath);
      }
      if(LinkageGDT)
         engine.IBDGDT();
      if(LinkageAncestry){
...
//         if(rplotFlag) plotAncestry(prefix);
      ...
      if(LinkagePOPDIST){
         ...
         if(rplotFlag) plotPopDist(prefix, rpath);
      }
      if(LinkageAUC){
         if(!positions.IsEmpty()){
            ...
            engine.AUCmapping();
            if(rplotFlag) plotAUCmapping(prefix, sexchr, rpath);
         }
      }
      if(LinkagePOPIBD){
...
//         if(rplotFlag) plotPopDist(prefix);
      ...
      if(LinkagePOPROH){
         ...
         if(rplotFlag) plotPopROH(prefix, sexchr, rpath);
      }
      if(LinkageIBDMI)
         engine.IBDMI();
      if(LinkageH2)
      ...
      if(LinkageROHMAP){
         ...
         if(rplotFlag) plotROHmapping(prefix, stratName, sexchr, rpath);
      }
      if(LinkageROHMAPQT){
         if(stratName!="")
         ...
         if(rplotFlag) plotROHforQT(prefix, sexchr, rpath);
      }
      if(LinkageVC)
         engine.IBDVC();
      if(LinkageGRM)
      ...
      if(LinkageMDS){
         if(projectionFlag)
            ...
            engine.IBDMDS();
         if(rplotFlag) {
            if(plotAncestry(prefix, rpath))   // Failed
               plotPopStructure(prefix, projectionFlag, rpath);
         }
      }
      exit(0);
   ...
   if(splitPed){
      ...
      if(rplotFlag) plotSplitped(prefix, rpath);
      exit(0);
   }
   if(PhaseFlag){
