...
void Engine::IBDMDS_Projection()
{
         ...
         if(overlapCount > 2000){
            ...
            for(int i = 0; i < idCount; i++){
               ...
               if(ped[id].ngeno >= MINSNPCOUNT){
                  int pop = HashKG.Integer(ped[id].pid);
                  if(ped[id].affections[0]==0 && pop>-1 && pop<6){
                     subset[0].Push(i);
            ...
            for(int i = 0; i < idCount; i++){
               ...
               if(ped[id].ngeno >= MINSNPCOUNT){
                  ...
                  if(ped[id].affections[0]==0 && pop>-1 && pop<4){
                     subset[0].Push(i);
                     fprintf(fp, "%s %s ", (const char*)ped[id].famid, (const char*)ped[id].pid);
                     switch(pop){
   ...
   for(int seg = 0; seg < segCount; seg++){
      ...
      for(int pair = 0; pair < allpairCount; pair++)
         pi[pair] += (ibdsegStorage[0][pair]*0.5 + ibdsegStorage[1][pair]);
   }  // end of seg loop
...
void Engine::IBDMDS_Internal(IntArray & subset, Vector & EigenValue, Matrix & EigenVector)
{
   ...
   for(int seg = 0; seg < segCount; seg++){
      ...
      #pragma omp parallel for num_threads(defaultMaxCoreCount)
      for(int pair = 0; pair < allpairCount; pair++)
         D[index[pair*2]][index[pair*2+1]] += (ibdsegStorage[0][pair]*0.5 + ibdsegStorage[1][pair]);
   }  // end of seg loop
...
void Engine::IBDVC()
{
   ...
   for(int seg = 0; seg < segCount; seg++){
      ...
      for(int p = 0; p < allpairCount; p++)
         pi[allpairs[p*2]][allpairs[p*2+1]] += (ibdsegStorage[0][p]*0.5 + ibdsegStorage[1][p]);
   }  // end of seg loop
   for(int i = 0; i < idCount; i++)
      for(int j = i+1; j < idCount; j++)
         pi[i][j] /= totalLength;
...
void Engine::IBDMDS()
{
   ...
   for(int seg = 0; seg < segCount; seg++){
      ...
      #pragma omp parallel for num_threads(defaultMaxCoreCount)
      for(int pair = 0; pair < allpairCount; pair++)
         D[allpairs[pair*2]][allpairs[pair*2+1]] += (ibdsegStorage[0][pair]*0.5 + ibdsegStorage[1][pair]);
   }  // end of seg loop
   for(int i = 0; i < idCount; i++)
      for(int j = i+1; j < idCount; j++)
         D[i][j] /= totalLength;
      ...
      for(int p1 = 0; p1 < popCount; p1++)
         for(int p2 = p1; p2 < popCount; p2++){
            ...
            for(int pair = 0; pair < pairCount; pair++)
               temp += (ibdsegStorage[0][pair]*0.5 + ibdsegStorage[1][pair]);
            popdist[index] += temp;
            index++;
         }  // end of p2 loop
   }  // end of seg loop
...
#ifdef _OPENMP
   #pragma omp parallel num_threads(defaultMaxCoreCount) \
      private(id1, id2, word, segstart, thread, \
