...
void GetGeneration(Family *fp, IntArray & Generation)
{
   int ncount = fp->count;
   Generation.Dimension(ncount);
   Generation.Zero();
   for(int i = 0; i < fp->founders; i++) Generation[i] = 1;
   for(int i = fp->founders; i < ncount; i++){
      int fa = fp->ped[fp->path[i]].father->traverse;
      int mo = fp->ped[fp->path[i]].mother->traverse;
      if(Generation[fa] == Generation[mo])
         Generation[i] = Generation[fa]+1;
      else{
         int smaller = fa;
         int larger = mo;
         if(Generation[fa] > Generation[mo]){smaller = mo; larger = fa;}
         if(smaller < fp->founders){
            Generation[i] = Generation[larger]+1;
            Generation[smaller] = Generation[larger];
         }else{
            printf("Please finish implementing the scenario when the two parents are in different generations and none are founders\n");
            printf("Error: parent %s is not a founder with parents %s and %s\n",
               (const char*)fp->ped[fp->path[smaller]].pid,
               (const char*)fp->ped[fp->path[smaller]].fatid,
               (const char*)fp->ped[fp->path[smaller]].motid);
            exit(0);
         }
      }
   }
   IntArray counts(100); counts.Zero();
   for(int i = 0; i < ncount; i++){
      int g = Generation[i];
      counts[g]++;
      Generation[i] = (g << 16) | counts[g];
   }                                        
}
...
//         GetGeneration(pf, Generation); // algorithm to be improved
         for(int i = 0; i < pf->count; i++){
            int id = pf->path[i];
            fprintf(fp, "%s %s %s %s %s %s %d %d %d",
               (const char*)pf->famid, (const char*)ped[id].pid,
               (const char*)pf->famid, (const char*)ped[id].pid,
               (const char*)ped[id].fatid, (const char*)ped[id].motid,
               ped[id].sex, ped.affectionCount? ped[id].affections[0]: 0, ped[id].ngeno>0? 0: 1);
//            int g = Generation[i]; fprintf(fp, " %.4lf", (g>>16)+(g&0xFFFF)*0.0001);
            fprintf(fp, "\n");
         }
      }
   }
...
int Engine::BuildOneFamily(int f, IntArray chrSeg, double totalLength, String & message)
{
   ...
   if(IBDvalidFlag && Bit64==64){
      IBDSegInSubset64Bit(pairs, ibdprops, maxLengths, ibd2props, maxLengths2, LG, markerCount, chrSeg, totalLength, bp, ibd1segs);
   ...
   for(int j = 0; j < fsCount; j++){
      ...
      if(ss[0] > -1 && ss[1] > -1){ // RULE FS2
         ...
         for(int s = 0; s < 2; s++)
            if(pa[s] > -1) parents[s][smaller] = pa[s] - pf->first;
         ...
         for(int s = 0; s < 2; s++)
            if(pa[s] > -1) parents[s].Push(pa[s] - pf->first);
...
void Engine::internalKING(int degree)
{
   ...
   for(int t = 0; t < defaultMaxCoreCount; t++){
      ...
      if(Bit64==32 || !IBDvalidFlag){
         ...
         IBDSegInSubset64Bit(allpairs[t], ibdprops, maxLengths, ibd2props, maxLengths2, LG, markerCount, chrSeg, totalLength, bp);
      for(int p = 0; p < pairCount; p++){
         if(!het1Counts[p] && !het2Counts[p] && !HetHetCounts[p]) continue;
         id1 = allpairs[t][p*2];
         id2 = allpairs[t][p*2+1];
   ...
   if(Bit64==32 || !IBDvalidFlag){
      if(errorrateCutoff == _NAN_){
         ...
         printf("Cutoff value for IBS0 between FS and PO is set at %.4f\n", errorrateCutoff);
      }
      for(int i = 0; i < IBS0L1.Length(); i++){
...
int Engine::ClusterFamily(int pedrebuildFlag, int degree)
{
   ...
   if (uniqueIID) printf("Individual IDs are unique across all families.\n");
   ...
   int maxCount = 1024;
   IntArray *cluster = new IntArray[maxCount];
   int clusterCount = 0;
   String temp1, temp2;
      ...
      for(int i = 0; i < Lr.Length()/2; i++){
         ...
         if(exist[0] == -1 && exist[1] == -1){
            if (clusterCount >= maxCount) {
               IntArray *tempcluster = new IntArray[maxCount*2];
               for (int i = 0; i < maxCount; i++)
                   tempcluster[i] = cluster[i];
               delete[]cluster;
               cluster = tempcluster;
               maxCount <<= 1;
            }
      ...
      if(pedrebuildFlag==0){ // clustering only
         ...
         if(Bit64 == 64 && totalLength > 10000000){
            ...
            for(int c = 0; c < clusterCount; c++){
               ...
               if(pairCount==0) continue;
               KinshipInSubset64Bit(pairs, HetHetCounts, IBS0Counts, het1Counts, het2Counts, HomHomCounts, IBSCounts);
               IBDSegInSubset64Bit(pairs, ibdprops, maxLengths, ibd2props, maxLengths2, LG, markerCount, chrSeg, totalLength, bp);
               for(int p = 0; p < pairCount; p++){
                  if(!het1Counts[p] && !het2Counts[p] && !HetHetCounts[p]) continue;
   ...
   int FIDCount = FID.Length();
   int sampleCount = sampleName.Length();
   if(FIDCount == 0){ // read from Merlin format input
   ...
   if((pedrebuildFlag==1) && clusterCount)   // option --build, save update-ids file
      fp = fopen((const char*)updateidsfile, "wt");   
   if (FIDCount >= sampleCount) {
       if (ped.haveTwins) {
           for (int i = 0; i < sampleCount; i++) 
               if (ped[phenoid[i]].zygosity) ZG.Push("MZ"); else ZG.Push("X");
           for (int i = sampleCount; i < FIDCount; i++) ZG.Push("X");
       }
       for (int i = 0; i < clusterCount; i++) {
           newName = "KING";
           newName += (i + 1);
