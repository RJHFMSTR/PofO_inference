...
void KingEngine::WritePlinkBinary(const char *prefix)
{
   ...
   if(Bit64==64){
      ...
      for(int m = 0; m < xmarkerCount; m++){
         int b = (m >> 6);
         int k = (m & 0x3F);
         unsigned long long int longbase=(1<<k);
         for(int i = 0; i < (idCount-removeCount+extraCount-1)/4+1; i++)
            alocus[i] = 0;
         byte = bit = 0;
         for(int f = 0; f < ped.familyCount; f++)
            for(int i = 0; i < id[f].Length(); i++){
               if(removeflags[id[f][i]]) continue;
               if(XLG[0][geno[id[f][i]]][b] & longbase)
                  g = (XLG[1][geno[id[f][i]]][b] & longbase) ? 0: 3;
               else
                  g = (XLG[1][geno[id[f][i]]][b] & longbase)? 2: 1;
               if(g) alocus[byte] |= (g << bit);
               if(bit == 6){
                  bit = 0;
                  byte ++;
      ...
      for(int m = 0; m < ymarkerCount; m++){
         ...
         for(int f = 0; f < ped.familyCount; f++)
            for(int i = 0; i < id[f].Length(); i++){
               if(removeflags[id[f][i]]) continue;
               if(YLG[0][geno[id[f][i]]][b] & longbase)
                  g = (YLG[1][geno[id[f][i]]][b] & longbase) ? 0: 3;
               else
                  g = (YLG[1][geno[id[f][i]]][b] & longbase)? 2: 1;
               if(g) alocus[byte] |= (g << bit);
               if(bit == 6){
                  bit = 0;
                  byte ++;
      ...
      for(int m = 0; m < mtmarkerCount; m++){
         ...
         for(int f = 0; f < ped.familyCount; f++)
            for(int i = 0; i < id[f].Length(); i++){
               if(removeflags[id[f][i]]) continue;
               if(MLG[0][geno[id[f][i]]][b] & longbase)
                  g = (MLG[1][geno[id[f][i]]][b] & longbase) ? 0: 3;
               else
                  g = (MLG[1][geno[id[f][i]]][b] & longbase)? 2: 1;
               if(g) alocus[byte] |= (g << bit);
               if(bit == 6){
                  bit = 0;
                  byte ++;
...
/*
void KingEngine::ComputeMZBySNP64Bit(IntArray &nonmissingMZCounts, IntArray &ibs1MZCounts, IntArray &HetHetMZCounts, IntArray &ibs0MZCounts, unsigned long long int **localLG[2], int localmarkerCount)
{
...
void KingEngine::ComputeTrioBySNP64Bit(IntArray &MItrioCounts, IntArray &HetInOffspringCounts, IntArray &nonmissingtrioCounts, unsigned long long int **localLG[2], int localmarkerCount)
{
   int locallongCount = (localmarkerCount-1)/64+1;
   const int BLOCKSIZE=255;
...
void KingEngine::ComputePOBySNP64Bit(IntArray &HomHomCounts, IntArray &ibs0Counts, IntArray &nonmissingCounts, unsigned long long int **localLG[2], int localmarkerCount)
{
   int locallongCount = (localmarkerCount-1)/64+1;
   const int BLOCKSIZE=255;
   const int CACHESIZE=16; // total cache size: 2^(6 + 4 - 2 + 8 + 1) = 2^17 = 128KB
...
void KingEngine::ComputeAlleleFrequency64Bit(IntArray &AACounts, IntArray &AaCounts, IntArray &missingCounts, unsigned long long int **localLG[2], int localmarkerCount)
{
   const int BLOCKSIZE=255;
