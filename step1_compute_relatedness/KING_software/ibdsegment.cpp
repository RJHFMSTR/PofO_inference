...
void Engine::ROH()
{
	...
	Vector rohprops, maxLengths, rohpropsX, maxLengthsX;
   IntArray *allsegments = new IntArray[idCount];
	if(totalLength > 0.0)
      ROH64BitMain(LG, markerCount, chrSeg, totalLength, bp, rohprops, maxLengths, allsegments);
	IntArray *allsegmentsX = new IntArray[idCount];
   if (totalLengthX > 0.0) {
       ROH64BitMain(XLG, xmarkerCount, chrSegX, totalLengthX, xbp, rohpropsX, maxLengthsX, allsegmentsX);
       // identify SNPs with no male coverage, e.g., coverage < 95%
	...
	if (ped.affectionCount)
		fprintf(fp, "\t%s", ped.affectionNames[0] == "DISEASEKING" ? "DISEASE" : (const char*)ped.affectionNames[0]);
	fprintf(fp, "\tMaxROH\tF_ROH");
   if (totalLengthX > 50000000.0) {
      fprintf(fp, "\tF_ROH_X");
      if (ymarkerCount > 100) fprintf(fp, "\tNGENO_Y\tSexChr");
      fprintf(fp, "\tInfSex");
      if (ymarkerCount > 100) fprintf(fp, "\tSexErr");
   }
   fprintf(fp, "\n");
	for (int id = 0; id < idCount; id++) {
		...
		fprintf(fp, "\t%.1lf\t%.4lf", maxLengths[id] * 0.000001, rohprops[id]);
      if (totalLengthX > 50000000.0) {
          int NX = -1, NY = 0;
          fprintf(fp, "\t%.4lf", rohpropsX[id]);
          if (rohpropsX[id] > 0.65) NX = 1;  // X, likely MALE
          else if (rohpropsX[id] < 0.3) NX = 2; // XX, likely FEMALE
          int yCount = 0;
          if (ymarkerCount > 100) {
             for (int m = 0; m < ylongCount; m++)
                yCount += popcount(YLG[0][id][m] | YLG[1][id][m]);
             fprintf(fp, "\t%d", yCount);
             NY += (yCount > ymarkerCount / 2);
             if (NX == 1 && NY == 1) fprintf(fp, "\tXY\tMALE");
             else if (NX == 2 && NY == 0) fprintf(fp, "\tXX\tFEMALE");
             else if (NX == 1 && NY == 0) fprintf(fp, "\tXO\tFEMALE");
             else if (NX == 2 && NY == 1) fprintf(fp, "\tXXY\tMALE");
             else if (NY == 0) fprintf(fp, "\tNA\tFEMALE");
             else if (NY == 1) fprintf(fp, "\tNA\tMALE");
             fprintf(fp, "\t%d", (2 - NY) == ped[phenoid[id]].sex ? 0: (ped[phenoid[id]].sex ? 1 : -1));
          }else              
             fprintf(fp, "\t%s", NX == 1 ? "MALE" : (NX == 2 ? "FEMALE" : "NA"));
      }
      fprintf(fp, "\n");
	...
	printf("  Short ROHs (<5Mb) are discarded.\n");
	...
	if (allsegments == NULL) error("allsegments not allocated");
	for (int id = 0; id < idCount; id++) {
		int allsegmentsCount = allsegments[id].Length() / 2;
		if (allsegmentsCount) {
			sprintf(tempbuffer, "%s\t%s",
				(const char*)ped[phenoid[id]].famid, (const char*)ped[phenoid[id]].pid);
			for (int s = 0; s < allsegmentsCount; s++) {
				segstart = allsegments[id][s * 2];
				segstop = allsegments[id][s * 2 + 1];
				pbuffer += sprintf(&buffer[pbuffer], "%s\t%d\t%.3lf\t%.3lf\t%s\t%s\t%d\t%.1lf\n",
					tempbuffer, chromosomes[segstart], bp[segstart] * 0.000001, bp[segstop] * 0.000001,
      ...
      int allsegmentsXCount = totalLengthX > 0.0 ? allsegmentsX[id].Length() / 2 : 0;
		if(allsegmentsXCount){
		   sprintf(tempbuffer, "%s\t%s", (const char*)ped[phenoid[id]].famid, (const char*)ped[phenoid[id]].pid);
		   for (int s = 0; s < allsegmentsXCount; s++) {
			   segstart = allsegmentsX[id][s * 2];
			   segstop = allsegmentsX[id][s * 2 + 1];
			   pbuffer += sprintf(&buffer[pbuffer], "%s\t%d\t%.3lf\t%.3lf\t%s\t%s\t%d\t%.1lf\n",
				    tempbuffer, 23, xbp[segstart] * 0.000001, xbp[segstop] * 0.000001,
				    (const char*)xsnpName[segstart], (const char*)xsnpName[segstop],
				    segstop - segstart + 1, (xbp[segstop] - xbp[segstart])*0.000001);
		   }
			if (pbuffer > 0xFFFF) {  // buffer big enough for writing
				gzwrite(fps, buffer, pbuffer);
				pbuffer = 0;
...
void Engine::ROH64BitMain(unsigned long long int **localLG[2], int localmarkerCount, IntArray &localchrSeg, double localtotalLength, IntArray &localbp, Vector &rohprops, Vector &maxLengths, IntArray *allsegments)
{
	int locallongCount = (localmarkerCount - 1) / 64 + 1;
	int segCount = localchrSeg.Length() / 4;
	int cCount, icCount, segstart, segstop;
	unsigned long long int word;
	rohprops.Dimension(idCount);
	rohprops.Zero();
	maxLengths.Dimension(idCount);
	maxLengths.Zero();
#ifdef __ZLIB_AVAILABLE__
	for (int id = 0; id < idCount; id++)
...
#pragma omp for
		...
		for (int id = 0; id < idCount; id++) {
			...
			for (int seg = 0; seg < segCount; seg++) {  // go through each segment
				int chrsegMin = localchrSeg[seg << 2];
				int chrsegMax = localchrSeg[(seg << 2) | 1];
				int minExtraBit = localchrSeg[(seg << 2) | 2];
				int maxExtraBit = localchrSeg[(seg << 2) | 3];
				...
				for (int m = chrsegMin; m <= chrsegMax; m++) {
					for (; (m <= chrsegMax) &&  // keep passing if m word includes IC (Aa) or does not include any C (AA)
						(((~localLG[0][id][m]) & localLG[1][id][m]) || (localLG[0][id][m] & localLG[1][id][m]) == 0);
						m++); //includes IC (Aa) OR does not include any C (AA)
					for (cCount = 0, segstart = m; m <= chrsegMax &&
						((~localLG[0][id][m]) & localLG[1][id][m]) == 0; m++)
						cCount += popcount(localLG[0][id][m] & localLG[1][id][m]);
					if (cCount < 10 || localbp[((m - 1) << 6) | 0x3F] - localbp[segstart << 6] < 200000)
						continue;   // continue only when cCount>=10 AND >0.2Mb
					for (segstart--; segstart >= chrsegMin &&  // icCount==0
						((~localLG[0][id][segstart]) & localLG[1][id][segstart]) == 0; segstart--);
				...
				for (int t = 0; t < tempcount - 1; t++) {
					int gap = localbp[tempStart[t + 1] << 6] - localbp[(tempStop[t] << 6) | 0x3F];
					if (tempStart[t + 1] - tempStop[t] < 3) {
						...
						for (int m = tempStop[t] + 1; m < tempStart[t + 1]; m++) {
							icCount += popcount((~localLG[0][id][m]) & localLG[1][id][m]);
							cCount += popcount(localLG[0][id][m] & localLG[1][id][m]);
						}
						if (cCount > icCount * 3) // merge if C_roh > 75%
							tempStart[t + 1] = tempStart[t];
						else if (localbp[(tempStop[t] << 6) | 0x3F] - localbp[tempStart[t] << 6] > 5000000) {
				...
				if (localbp[(tempStop[tempcount - 1] << 6) | 0x3F] - localbp[tempStart[tempcount - 1] << 6] > 5000000) {
				...
				for (int t = 0; t < tempcount; t++) {
					...
					if (m > chrsegMin) {
						m--; // Aa
						word = localLG[1][id][m] & (~localLG[0][id][m]);
						int bit = 0;
						for (; bit < 63 && (word & ((unsigned long long int)1 << (63 - bit))) == 0; bit++);
					...
					if (m < chrsegMax) {
						...
						word = localLG[1][id][m] & (~localLG[0][id][m]);
						int bit = 0;
						for (; bit < 63 && (word & ((unsigned long long int)1 << bit)) == 0; bit++);
			...
			for (int seg = 0; seg < newsegCount; seg++) {
				...
				if (stopPos[seg] == locallongCount - 1)
					segstop = localmarkerCount - 1;
				else
					segstop = ((stopPos[seg] << 6) | 0x3F) + stopExtraBit[seg];
				length = localbp[segstop] - localbp[segstart];
				if (length > maxLength)
					maxLength = length;
				if (length > 5000000) {
					prop += length;
			...
			prop /= localtotalLength;
			rohprops[id] = prop;
			maxLengths[id] = maxLength;
#ifdef __ZLIB_AVAILABLE__
			if (prop > 0.005 || localmarkerCount==xmarkerCount)
...
void Engine::ComputeIBDSegment64Bit()
{
   ...
   ComputeIBDSegmentMain64Bit(LG, markerCount, chrSeg, totalLength, bp);
   if(totalLengthX>0.0)
      ComputeIBDSegmentMain64Bit(XLG, xmarkerCount, chrSegX, totalLengthX, xbp);
   ...
   if(totalLengthX>0.0){
      outfile=prefix;
      outfile.Add("X.seg");
      printf("Additional summary statistics of X-Chr IBD segments saved in file %s\n", (const char*)outfile);
   }
#ifdef __ZLIB_AVAILABLE__
   ...
   if(totalLengthX>0.0){
      outfile2=prefix;
      outfile2.Add("X.segments.gz");
      printf("Additional X-Chr IBD segments saved in a gzipped file %s\n", (const char*)outfile2);
   }
#endif
}

void Engine::ComputeIBDSegmentMain64Bit(unsigned long long int **localLG[2], int localmarkerCount, IntArray &localchrSeg, double localtotalLength, IntArray &localbp)
{
   int locallongCount = (localmarkerCount-1)/64+1;
   bool IsAutosome = (localLG==LG);
   int m1, m3;
   unsigned long long int word;
   int *missingCount = new int[idCount];
   int extramissingCount = (locallongCount<<6)-localmarkerCount;
#ifdef _OPENMP
   #pragma omp parallel for num_threads(defaultMaxCoreCount) private(m1, m3, word)
#endif
   for(int i = 0; i < idCount; i++){
      for(m3 = m1 = 0; m1 < locallongCount; m1++)   // not all non-missing
         for(word = ~(localLG[0][i][m1] | localLG[1][i][m1]); word; word &= (word-1), m3++);
      missingCount[i] = m3 - extramissingCount;
   }  // parallel among individuals ends
   int cutoffMissingCount=localmarkerCount-MINSNPCOUNT;
   if(localmarkerCount * 0.2 > cutoffMissingCount) cutoffMissingCount = int(localmarkerCount * 0.2);
   ...
   if(projValid && projectStart==0 && ped.affectionCount==0){
      ...
      printf("  or count of the first part (N) can be specified as --projection N.\n");
      return;
   }
   IntArray subsetRef(0), subsetProj(0);
   if(projectStart){ // --ibdseg --proj N
      ...
      for(int i = projectStart; i < idCount; i++)
         if(missingCount[i] < cutoffMissingCount)
            subsetProj.Push(i);
   }else if(projectFlag){  // --ibseg --proj
      for(int i = 0; i < idCount; i++){
         int id = phenoid[i];
         if(missingCount[i] < cutoffMissingCount){
      ...
      for(int i = 0; i < idCount; i++)
         if(missingCount[i] < cutoffMissingCount) subsetRef.Push(i);
         else printf("Sample FID=%s,IID=%s is removed with only %d non-missing SNPs.\n",
            (const char*)ped[phenoid[i]].famid, (const char*)ped[phenoid[i]].pid,
            localmarkerCount - missingCount[i]);
   ...
   if(projectStart)
      ...
      printf("Only pairs between %d unaffected and %d affected individuals are inferred.\n",
         refCount, projCount);
   int segCount = localchrSeg.Length()>>2;
   int segstart, segstop;
...
#ifdef _OPENMP
   printf("%d CPU cores are used for %s inference...\n",
      defaultMaxCoreCount, IsAutosome?"autosome":"X-chr");
#endif
#ifdef __ZLIB_AVAILABLE__
   const int coreCount = defaultMaxCoreCount;
   gzFile fps[coreCount];
   String outfile2(prefix);
   if(IsAutosome) outfile2.Add(".segments.gz");
   else outfile2.Add("X.segments.gz");
   ...
   #pragma omp parallel num_threads(defaultMaxCoreCount) \
      ...
      thread, maxLength, prop, pbuffer, buffer)
{
#endif
...
"FID1\tID1\tFID2\tID2\tIBDType\tChr\tStartMB\tStopMB\tStartSNP\tStopSNP\t\tN_SNP\tLength\n");
      ...
      for(int seg = 0; seg < segCount; seg++){
         ...
         for(int i1 = i; i1 < iMax; i1++){
            ...
            for(int i2 = jMin; i2 < jMax; i2++){
               ...
               minExtraBit = localchrSeg[(seg<<2)|2];
               maxExtraBit = localchrSeg[(seg<<2)|3];
               for(localMin = chrsegMin; localMin <= chrsegMax; localMin++){
                  for(; (localMin <= chrsegMax) &&
                  (((localLG[0][id1][localMin] & localLG[0][id2][localMin] &
                  (localLG[1][id1][localMin] ^ localLG[1][id2][localMin])) || //AA x aa
                  ((localLG[1][id1][localMin] | localLG[1][id2][localMin])==0))); // Neither is AA or Aa
                  localMin++); // keep passing if localMin includes AA x aa
                  if((localMin < chrsegMax) &&
                  (localLG[0][id1][localMin+1] & localLG[0][id2][localMin+1] &
                  (localLG[1][id1][localMin+1] ^ localLG[1][id2][localMin+1]))==0)
                     break;   // get out of the loop only when two 0 AA x aa words in a row
               }
               if(localMin >= chrsegMax ||
                  localbp[(chrsegMax<<6)|0x3F] - localbp[localMin<<6] < 2500000)
                  continue;// pass if the segment is shorter than 2.5MB
               if(localMin > chrsegMin){
                  word = localLG[0][id1][localMin-1] & localLG[0][id2][localMin-1] & (localLG[1][id1][localMin-1] ^ localLG[1][id2][localMin-1]);
                  if(word == 0){
                     localMin --;
                     minExtraBit = 0;
                  }else
                     for(minExtraBit = 0; (word & ((unsigned long long int)1<<(63-minExtraBit))) == 0; minExtraBit++);
               }
               for(localMax = chrsegMax; localMax >= localMin; localMax--){
                  for(; (localLG[0][id1][localMax] & localLG[0][id2][localMax] &
                     (localLG[1][id1][localMax] ^ localLG[1][id2][localMax])) ||  //AA x aa
                     ((localLG[1][id1][localMax] | localLG[1][id2][localMax])==0);//None is AA or Aa
                     localMax--); // keep passing if localMax includes AA x aa
                  if((localMax > localMin) &&
                     (localLG[0][id1][localMax-1] & localLG[0][id2][localMax-1] &
                     (localLG[1][id1][localMax-1] ^ localLG[1][id2][localMax-1]))==0)
                     break;   // get out of the loop only when two 0 AA x aa words in a row
               }
               if(localMax < localMin + 2 ||   // 1 or 2 words are not enough for an IBD segment
                  localbp[(localMax<<6)|0x3F] - localbp[localMin<<6] < 2500000)
                  continue;// pass if the segment is shorter than 2.5MB
               if(localMax < chrsegMax){
                  word = localLG[0][id1][localMax+1] & localLG[0][id2][localMax+1] & (localLG[1][id1][localMax+1] ^ localLG[1][id2][localMax+1]);
                  if(word == 0){
                     localMax ++;
                     maxExtraBit = 0;
                  }else
                     for(maxExtraBit = 0; (word & ((unsigned long long int)1<<maxExtraBit)) == 0; maxExtraBit++);
               }
               ...
               for(int m = localMin; m <= localMax; m++){
                  ...
                  ((((localLG[0][id1][m] ^ localLG[0][id2][m]) | (localLG[1][id1][m] ^ localLG[1][id2][m])) &
                  (localLG[0][id1][m] | localLG[1][id1][m]) & (localLG[0][id2][m] | localLG[1][id2][m])) ||
                  (localLG[1][id1][m] & localLG[1][id2][m] & (~(localLG[0][id1][m]^localLG[0][id2][m])))==0);
                  ...
                  ((((localLG[0][id1][m] ^ localLG[0][id2][m]) | (localLG[1][id1][m] ^ localLG[1][id2][m])) &
                  (localLG[0][id1][m] | localLG[1][id1][m]) & (localLG[0][id2][m] | localLG[1][id2][m]))) == 0; m++)
                     cCount += popcount(localLG[1][id1][m] & localLG[1][id2][m] & (~(localLG[0][id1][m]^localLG[0][id2][m])));
                  if(cCount < 10 || localbp[((m-1)<<6)|0x3F] - localbp[segstart<<6] < 100000)
                     continue;   // continue only when cCount>=10 AND >0.2Mb
                  for(word=0; (segstart >= localMin) && (word==0 || (word & (word-1)) == 0); segstart--)
                     word = (((localLG[0][id1][segstart] ^ localLG[0][id2][segstart]) |
                        (localLG[1][id1][segstart] ^ localLG[1][id2][segstart])) &
                        (localLG[0][id1][segstart] | localLG[1][id1][segstart]) &
                        (localLG[0][id2][segstart] | localLG[1][id2][segstart]));// IC
                  ...
                  for(word = 0, m--; (m <= localMax) && (word == 0 || (word & (word-1)) == 0); m++)
                     word = (((localLG[0][id1][m] ^ localLG[0][id2][m]) |
                        (localLG[1][id1][m] ^ localLG[1][id2][m])) &
                        (localLG[0][id1][m] | localLG[1][id1][m]) &
                        (localLG[0][id2][m] | localLG[1][id2][m]));// IC
               ...
               if(!skipFlag){
                  ...
                  for(int t = 0; t < tempcount-1; t++){
                     ...
                     if(tempStart[t+1] - tempStop[t] < 3){
                        ...
                        for(int m = tempStop[t]+1; m < tempStart[t+1]; m++){
                           icCount += popcount((((localLG[0][id1][m] ^ localLG[0][id2][m]) | (localLG[1][id1][m] ^ localLG[1][id2][m])) &
                              (localLG[0][id1][m] | localLG[1][id1][m]) & (localLG[0][id2][m] | localLG[1][id2][m])));
                           cCount += popcount(localLG[1][id1][m] & localLG[1][id2][m] & (~(localLG[0][id1][m]^localLG[0][id2][m])));
                        }
                        if(cCount > icCount*3) // merge if C_IBD2 >
                           tempStart[t+1] = tempStart[t];
                        else if(localbp[(tempStop[t]<<6)|0x3F] - localbp[tempStart[t]<<6] >= 2500000){
                  ...
                  if(localbp[(tempStop[tempcount-1]<<6)|0x3F] - localbp[tempStart[tempcount-1]<<6] >= 2500000){
                  ...
                  if(tempcount == 0){  // no IBD2 segments
                     ...
                     for(int t = 0; t < tempcount; t++){
                        if(mergedStart[t] == localMin)
                           ...
                           word = ((localLG[0][id1][m] ^ localLG[0][id2][m]) | (localLG[1][id1][m] ^ localLG[1][id2][m]))
                              & (localLG[0][id1][m] | localLG[1][id1][m]) & (localLG[0][id2][m] | localLG[1][id2][m]);
                           int bit = 0;
                           for(; bit < 63 && (word & ((unsigned long long int)1<<(63-bit))) == 0; bit++);
                           mergedBit.Push(bit);
               ...
               for(int s = 0; s < newcount; s++){
                  ...
                  for(int m = chrsegMin; m <= chrsegMax; m++){
                     ...
                     for(cCount = 0, segstart = m; m <= chrsegMax &&
                        (localLG[0][id1][m] & localLG[0][id2][m] & (localLG[1][id1][m] ^ localLG[1][id2][m]))==0; m++)
                        cCount += popcount(localLG[1][id1][m] & localLG[1][id2][m] & (localLG[0][id1][m] | localLG[0][id2][m]));
                     if(cCount < 10 || localbp[((m-1)<<6)|0x3F] - localbp[segstart<<6] < 200000)
                        continue;   // continue only when cCount>=10 AND >0.2Mb
                     for(segstart--; segstart >= chrsegMin &&  // icCount==0
                        (localLG[0][id1][segstart] & localLG[0][id2][segstart] & (localLG[1][id1][segstart] ^ localLG[1][id2][segstart]))==0;
         ...
         for(int i2 = jMin; i2 < jMax; i2++){
            ...
            for(int k = 0; k < 2; k++){
               ...
               for(int seg = 0; seg < newsegCount; seg++){
                  ...
                  if(stopPos[k][ii][jj][seg] == locallongCount-1)
                     segstop = localmarkerCount - 1;
                  else
                     segstop = ((stopPos[k][ii][jj][seg]<<6)|0x3F)+stopExtraBit[k][ii][jj][seg];
                  double length = localbp[segstop] - localbp[segstart];
            ...
            allsegs[thread].Push((int(maxLength[0]*0.00001+0.5)<<16) | int(prop[0]*10000+0.5));
            allsegs[thread].Push((int(maxLength[1]*0.00001+0.5)<<16) | int(prop[1]*10000+0.5));
#ifdef __ZLIB_AVAILABLE__
            ...
            for(int k = 0; k < 2; k++)
               for(int s = 0; s < interval[k].Length()/2; s++){
                  ...
                  pbuffer += sprintf(&buffer[pbuffer], "\t%d\t%.3lf\t%.3lf\t%s\t%s\t%d\t%.1lf\n",
                     23, xbp[segstart]*0.000001, xbp[segstop]*0.000001,
                     (const char*)xsnpName[segstart], (const char*)xsnpName[segstop],
                     segstop-segstart+1, (xbp[segstop]-xbp[segstart])*0.000001);
               }
            if(pbuffer > 0xFFFF){  // buffer big enough for writing
   ...
   int items[2];
   String outfile(prefix);
   if(IsAutosome) outfile.Add(".seg");
   else outfile.Add("X.seg");
   ...
   for(int c = 0; c < defaultMaxCoreCount; c++){
      ...
      for(int i = 0; i < allsegCount; i++){
         ...
         pbuffer += sprintf(&buffer[pbuffer],
            "%s\t%s\t%s\t%s\t%d\t%d\t%.1lf\t%.1lf\t%.4lf\t%.4lf\t%.4lf\t",
            (const char*)ped[phenoid[id1]].famid, (const char*)ped[phenoid[id1]].pid,
            (const char*)ped[phenoid[id2]].famid, (const char*)ped[phenoid[id2]].pid,
            ped[phenoid[id1]].sex, ped[phenoid[id2]].sex,
            (items[0]>>16)*0.1,  (items[1]>>16)*0.1, prop[0], prop[1], pi);
         pbuffer += sprintf(&buffer[pbuffer], "\n");
         if(pbuffer > 0xFFFF){  // buffer big enough for writing
...
bool Engine::PreSegment(bool printFlag)
{
   char buffer[256];
   if(Bit64!=64){
      sprintf(buffer, "64-bit system is required.");
      segmessage = buffer;
      return false;
   }
   chrSeg.Dimension(0);
   ...
   if(xmarkerCount){
      chrSegX.Dimension(0);
      startPos = 0;
      chrSegX.Push(startPos);
      for(int m = startPos+1; m < xmarkerCount; m++)
         if(xbp[m] - xbp[m-1] > 1000000){  // gap too big
            chrSegX.Push(m-1);
            chrSegX.Push(m);
         }
      chrSegX.Push(xmarkerCount-1);
      tmpchrSeg.Dimension(0);
      chrsegCount = chrSegX.Length()/2;
      for(int seg = 0; seg < chrsegCount; seg++){
         int extraBit = chrSegX[seg*2]%bit;
         if(extraBit){
            startPos = (chrSegX[seg*2]>>bitshift)+1;
            extraBit = bit - extraBit;
         }else
            startPos = (chrSegX[seg*2]>>bitshift);
         tmpchrSeg.Push(startPos);
         tmpchrSeg.Push(((chrSegX[seg*2+1]+1)>>bitshift)-1);
         tmpchrSeg.Push(extraBit);
         extraBit = (chrSegX[seg*2+1]+1)%bit;
         tmpchrSeg.Push(extraBit);
      }
      chrSegX.Dimension(0);
      chrsegCount = tmpchrSeg.Length()/4;
      for(int seg = 0; seg < chrsegCount; seg++){
         if(tmpchrSeg[seg*4+1] >= tmpchrSeg[seg*4] + wordcount){
            for(startPos = tmpchrSeg[seg*4]; xbp[(startPos+1)<<bitshift] - xbp[startPos<<bitshift] > 10000000; startPos++);
            for(stopPos = tmpchrSeg[seg*4+1]; xbp[(stopPos<<bitshift)-1] - xbp[((stopPos-1)<<bitshift)-1] > 10000000; stopPos--);
            if(stopPos - startPos + 1 < wordcount) continue;
            if(xbp[((stopPos+1)<<bitshift)-1] - xbp[startPos<<bitshift] < 10000000) continue; // only deal with large segments
            chrSegX.Push(startPos);
            chrSegX.Push(stopPos);
            if(startPos!=tmpchrSeg[seg*4]) chrSegX.Push(0);
            else chrSegX.Push(tmpchrSeg[seg*4+2]);
            if(stopPos!=tmpchrSeg[seg*4+1]) chrSegX.Push(0);
            else chrSegX.Push(tmpchrSeg[seg*4+3]);
         }
      }
   }
   String outfile;
   ...
   totalLengthX = 0.0;
   int chrsegCountX = chrSegX.Length()/4;
   for(int seg = 0; seg < chrsegCountX; seg++){
      startPos = (chrSegX[seg*4]<<bitshift)-chrSegX[seg*4+2];
      stopPos = ((chrSegX[seg*4+1]+1)<<bitshift)-1+chrSegX[seg*4+3];
      sprintf(buffer, "%d\t%d\t%.3lf\t%.3lf\t%.3lf\t%d\t%s\t%s\n",
         chrsegCount+seg+1, 23,
         xbp[startPos]*0.000001, xbp[stopPos]*0.000001, (xbp[stopPos] - xbp[startPos])*0.000001,
         stopPos - startPos + 1,
         (const char*)xsnpName[startPos], (const char*)xsnpName[stopPos]);
      fprintf(fp, "%s", buffer);
      totalLengthX += xbp[stopPos] - xbp[startPos];
   }
   fclose(fp);

   IntArray AACounts, AaCounts, missingCounts;
   IntArray subset(idCount);   for (int i = 0; i < idCount; i++) subset[i] = i;
   ComputeAlleleFrequency64Bit(subset, AACounts, AaCounts, missingCounts, LG, 64<<6, defaultMaxCoreCount);
   ...
   if(printFlag){
      printf("Total length of %d chromosomal segments usable for IBD segment analysis is %.1lf MB.\n",
         chrsegCount, totalLength*0.000001);
      if(totalLengthX>0)
         printf("  In addition to autosomes, %d segments of length %.1lf MB on X-chr can be further used.\n",
            chrsegCountX, totalLengthX*0.000001);
