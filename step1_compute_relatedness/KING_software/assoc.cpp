...
void Engine::CountTDT64Bit(IntArray &trioList, IntArray &tcounts, IntArray & ntcounts, unsigned long long int **localLG[2], int localmarkerCount)
{
   int locallongCount = 0;
   if(localmarkerCount==0) {
      localmarkerCount = markerCount;
      locallongCount = longCount;
   }else
      locallongCount = (localmarkerCount-1)/64+1;
   if(localLG==NULL){
      localLG[0] = LG[0];
      localLG[1] = LG[1];
   }
   int N_Trio = trioList.Length() / 3;
   tcounts.Dimension(locallongCount<<6);
   ntcounts.Dimension(locallongCount<<6);
   tcounts.Zero();
   ntcounts.Zero();
   if(N_Trio==0) return;
      ...
      private(nomiss, informative, informativePO, word, word2, trio, pchar, T, NT, thread)
{
   ...
   for(int k = 0; k < 2; k++){
      localcounts[k][thread] = new int [locallongCount<<6];   // allocate for effective thread
      ...
      for(int blockb = 0; blockb < locallongCount; blockb += CACHESIZE){  // blocked SNP words
         ...
         for(int i = bi; i < iMax; i++){
            ...
            for(int b = blockb; b < bMax; b++){
               nomiss = (localLG[0][trio[0]][b] | localLG[1][trio[0]][b]) &
                  (localLG[0][trio[1]][b] | localLG[1][trio[1]][b]) &
                  (localLG[0][trio[2]][b] | localLG[1][trio[2]][b]);
               informative = nomiss & ~(localLG[0][trio[0]][b] & localLG[0][trio[1]][b]);
               int bbb = ((b-blockb)<<3);
               word = informative & // Aa x Aa -> Aa
                  ~(localLG[0][trio[0]][b] | localLG[0][trio[1]][b] | localLG[0][trio[2]][b]);
               ...
               for(int p = 0; p < 2; p++){   // loop over two parents
                  informativePO = informative & (localLG[0][trio[p]][b] ^ localLG[0][trio[2]][b]);
                  word = informativePO & ((localLG[0][trio[2]][b] & localLG[1][trio[2]][b]) | (~localLG[1][trio[p]][b]));
                  T[bbb] += (word&0x0101010101010101);
                  for(int k = 1; k < 8; k++)
                     T[bbb|k] += ((word>>k)&0x0101010101010101);
                  word = informativePO & ((localLG[0][trio[p]][b] & localLG[1][trio[p]][b]) | (~localLG[1][trio[2]][b]));
                    ...
                    if (xflag)
                        for (int m = 0; m < xlongCount; m++) {
                            ...
                            het1Count += (popcount((XLG[0][id2][m] | XLG[1][id2][m]) & (~XLG[0][id1][m]) & XLG[1][id1][m])
                                + popcount((XLG[0][id1][m] | XLG[1][id1][m]) & (~XLG[0][id2][m]) & XLG[1][id2][m]));
...
void Engine::TDT()
{
   ...
   if(Bit64 == 64)
      CountTDT64Bit(trioList, tcounts, ntcounts, LG, markerCount);
   else
   ...
   if(Bit64==64)
      CountTDT64Bit(trioList, tcounts, ntcounts, XLG, xmarkerCount);
   else
...
void Engine::ExtractUnrelated(IntArray & unrelatedList, bool printFlag)
{
   unrelatedList.Dimension(0);
   SemifamilyKinship();
   IntArray unrelatedCounts, ID_unrelated, ID(0);
   QuickIndex idx;
   for(int f = 0; f < ped.familyCount; f++){
      int idfCount = id[f].Length();
      ID.Stack(id[f]);
      if(idfCount < 2) {
         if(idfCount == 1) unrelatedList.Push(id[f][0]);
         continue;  // no need to check relatedness in this family
      }
      unrelatedCounts.Dimension(idfCount);
      unrelatedCounts.Zero();
      ...
      idx.Index(unrelatedCounts);
      ID_unrelated.Dimension(0);
      ID_unrelated.Push(idx[unrelatedCounts.Length()-1]);
      for(int i = unrelatedCounts.Length()-2; (i >= 0) && (unrelatedCounts[idx[i]] > 0); i--){
   ...
   if (printFlag) {
      String file = prefix;  file.Add("unrelated.txt");
      FILE *fp = fopen((const char*)file, "wt");
      if (fp == NULL) error("Cannot open %s to write", (const char*)file);
      int unrelatedCount = unrelatedList.Length();
      for (int i = 0; i < unrelatedCount; i++)
         fprintf(fp, "%s\t%s\n", (const char*)ped[unrelatedList[i]].famid, (const char*)ped[unrelatedList[i]].pid);
      fclose(fp);
      printf("\nA list of %d unrelated individuals saved in file %s\n",
          unrelatedCount, (const char*)file);
      IntArray ID_remove = ID;     // generate a list of samples to be removed
      for (int i = 0; i < unrelatedCount; i++)
         ID_remove.Delete(ID_remove.Find(unrelatedList[i]));
      int removeCount = ID_remove.Length();
      file = prefix;  file.Add("unrelated_toberemoved.txt");
...
/*
void Engine::CountTDTinX64Bit(IntArray &trioList, IntArray &ncounts, IntArray & ntcounts)
{
   int N_Trio = trioList.Length() / 3;
   ncounts.Dimension(xlongCount*64);
   ncounts.Zero();
   ntcounts.Dimension(xlongCount*64);
   ntcounts.Zero();
   if(N_Trio==0) return;
   char revbase[65536];
   for(int i = 0; i < 16; i++)
